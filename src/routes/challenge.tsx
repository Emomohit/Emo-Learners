  ...
  head: () => ({
    meta: [
      { title: "30 Days Python Challenge — EMO Learners" },
      {
        name: "description",
        content:
          "Master Python in 30 days. Daily lessons, progress tracking, badges, and a certificate — follow CodeWithHarry's free course.",
      },
      { property: "og:title", content: "30 Days Python Challenge — EMO Learners" },
      {
        property: "og:description",
        content: "Build consistency. Master Python. Earn your certificate in 30 days.",
      },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          description:
            "30-day structured Python learning challenge by EMO Learners using CodeWithHarry's Complete Python Course.",
    ...
    <section className="relative overflow-hidden px-6 pt-20 pb-24 md:pt-28 md:pb-32">
      ...
      <div className="relative mx-auto max-w-6xl text-center reveal">
        ...
        <p className="mx-auto mt-6 max-w-2xl text-base text-white/70 md:text-lg">
          Build the habit. Master the language. Ship real projects. Follow CodeWithHarry's complete
          Python course in 30 focused days — with daily structure, streaks, badges, and a
        ...
        <div className="mx-auto mt-16 grid max-w-3xl grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div key={s.v} className="glass glass-hover p-5">
              ...
              <div className="mt-1 text-xs font-mono uppercase tracking-widest text-white/60">
                {s.v}
              </div>
            </div>
          ))}
  ...
  const items = [
    {
      ...
      desc: "Small daily reps beat marathon weekends. 30 days locks in the habit.",
    },
    {
      icon: Users,
      title: "Built-in Accountability",
      desc: "Track streaks, share progress, get nudged by the EMO community.",
    },
    {
      icon: BookOpen,
      title: "Structured Curriculum",
      desc: "Day-by-day topics mapped to CodeWithHarry's course. No guesswork.",
    },
  ];
  return (
    <section className="px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          ...
          title={
            <>
              Consistency is the <span className="grad-text">cheat code</span>
            </>
          }
        />
    ...
    <section className="px-6 py-20">
      <div className="mx-auto max-w-7xl">
        <SectionHeader
          ...
          title={
            <>
              30 Days. 30 <span className="grad-text">Wins.</span>
            </>
          }
          subtitle="Tap a day to read the notes, copy the snippet, and watch the matching CodeWithHarry chapter. Progress saves locally."
        />
              ...
              >
                ...
                >
                  {completed ? (
                    ...
                    <>
                      <Lock className="h-3 w-3" /> Mark done
                    </>
                  )}
                </button>
              </div>
...
function DayNotesDialog({
  day,
  onClose,
}: {
  day: (typeof CHALLENGE_DAYS)[number];
  onClose: () => void;
}) {
  useEffect(() => {
        ...
        <div className="max-h-[60vh] overflow-y-auto p-6">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-white/15 px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-white/70">
                ~{day.minutes} min
              </span>
            </div>
            <p className="mt-2 text-sm leading-relaxed text-white/85">{day.whatYouLearn}</p>
          </div>
          <h4 className="mt-6 font-mono text-[11px] uppercase tracking-widest text-primary">
            Key notes
          </h4>
          ...
          {day.snippet && (
            <>
              ...
              <pre className="mt-3 overflow-x-auto rounded-lg border border-white/10 bg-black/60 p-4 text-[12.5px] leading-relaxed text-emerald-200">
                <code>{day.snippet}</code>
              </pre>
            </>
          )}
        </div>
...
function Dashboard() {
  ...
  const message =
    completed === 0
      ...
      : completed >= 30
        ? "Legend. You finished. Now build something."
        : completed >= 21
          ? "Final stretch. Stay locked in."
          : completed >= 14
            ? "Halfway there. Momentum is on your side."
            : completed >= 7
              ? "One week in. The habit is forming."
    ...
    <section className="px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          ...
          title={
            <>
              Track every <span className="grad-text">streak</span>
            </>
          }
        />
        <div className="mt-12 grid gap-5 md:grid-cols-4">
          ...
          <Stat
            icon={Sparkles}
            label="Status"
            value={completed >= 30 ? "Done" : completed > 0 ? "Active" : "Not started"}
          />
        </div>
    ...
    <section className="px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          ...
          title={
            <>
              Unlock <span className="grad-text">milestone</span> badges
            </>
          }
          subtitle="Every checkpoint earns a badge. Show off your grind."
        />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {BADGES.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className={`glass glass-hover p-6 ${unlocked ? "grad-border" : "opacity-60"}`}
              >
                <div
                  className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${unlocked ? "grad-btn" : "bg-white/5"}`}
                >
                  {unlocked ? (
    ...
    <section className="px-6 py-20">
      <div className="mx-auto max-w-5xl">
        <SectionHeader
          ...
          title={
            <>
              Earn your <span className="grad-text">certificate</span>
            </>
          }
          subtitle="Finish all 30 days and unlock a shareable completion certificate."
        />
        <div className="mt-12 glass grad-border p-2 md:p-3">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, color-mix(in oklab, var(--c-blue) 14%, transparent), color-mix(in oklab, var(--c-purple) 14%, transparent))",
            }}
          >
            <div className="font-mono text-[10px] uppercase tracking-[0.4em] text-white/70">
              EMO Learners · Certificate of Completion
            </div>
            ...
            <p className="mt-2 grad-text font-display text-2xl font-bold md:text-3xl">
  ...
  const items = [
    {
      ...
      quote: "The daily structure made Python click for me. Streaks kept me honest.",
    },
    {
      name: "Priya K.",
      role: "BCA · final year",
      quote: "Best free challenge I've joined. The dashboard is genuinely motivating.",
    },
    {
      name: "Rohan M.",
      role: "12th passout",
      quote: "Started zero. Built my first project on Day 29. Worth every minute.",
    },
  ];
  return (
    <section id="community" className="px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          ...
          title={
            <>
              Stories from <span className="grad-text">past cohorts</span>
            </>
          }
        />
        <div className="mt-12 grid gap-5 md:grid-cols-3">
    ...
    <section id="faq" className="px-6 py-20">
      <div className="mx-auto max-w-3xl">
        <SectionHeader
          ...
          title={
            <>
              Quick <span className="grad-text">answers</span>
            </>
          }
        />
          ...
          {FAQ.map((f, i) => (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <ChevronDown
                  className={`h-5 w-5 shrink-0 text-white/60 transition-transform ${open === i ? "rotate-180" : ""}`}
                />
              </div>
        ...
        <div className="glass grad-border relative overflow-hidden p-10 text-center md:p-16">
          ...
          <div className="relative">
            ...
            <p className="mx-auto mt-4 max-w-xl text-white/70">
              Join hundreds of Indian students who are turning small daily reps into real Python
              skill.
            </p>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              <Link
                to={user ? "/dashboard" : "/auth"}
                className="grad-btn inline-flex items-center gap-2 rounded-full px-8 py-4 text-sm font-bold uppercase tracking-widest transition-transform hover:scale-105"
              >
                Join Now <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to="/join"
                className="glass glass-hover inline-flex items-center gap-2 rounded-full px-8 py-4 text-sm font-bold uppercase tracking-widest text-white"
              >
...
function SectionHeader({
  ...
  return (
    <div className="text-center">
      ...
      {subtitle && (
        <p className="mx-auto mt-4 max-w-2xl text-sm text-white/70 md:text-base">{subtitle}</p>
      )}
    </div>
  );
}
