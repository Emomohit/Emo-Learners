  ...
  head: () => ({
    meta: [
      { title: "AI Roadmap Generator — EMO Learners" },
      {
        name: "description",
        content:
          "Get a personalized week-by-week study roadmap for your branch, semester, and career goal — generated by AI.",
      },
      { property: "og:title", content: "AI Roadmap Generator — EMO Learners" },
      {
        property: "og:description",
        content: "Personalized week-by-week learning roadmaps for Indian engineering students.",
      },
    ],
  }),
...
function RoadmapPage() {
    ...
    try {
      ...
      try {
        localStorage.setItem(
          "emo:last-roadmap",
          JSON.stringify({ at: Date.now(), branch, semester, weeks, title: r?.title }),
        );
      } catch {}
  ...
  const summary = useMemo(() => {
    ...
    const totalOutcomes =
      result.milestones?.reduce((a, m) => a + (m.outcomes?.length ?? 0), 0) ?? 0;
  ...
  function buildMarkdown(r: RoadmapResult): string {
    ...
    if (r.projects?.length) {
      ...
      r.projects.forEach((p) =>
        lines.push(`- **${p.name}** — ${p.brief} _(stack: ${p.stack?.join(", ") ?? "—"})_`),
      );
      lines.push("");
    }
  ...
  function exportJson() {
    if (!result) return;
    download(
      `roadmap-${branch}-sem${semester}.json`,
      JSON.stringify({ meta: { branch, semester, goal, hours, weeks }, ...result }, null, 2),
      "application/json",
    );
      ...
      <section className="relative overflow-hidden px-4 pb-10 pt-16">
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-muted-foreground">
            Tell us your branch, semester, and goal. Get a week-wise roadmap with topics, projects,
            skills, and placement prep milestones.
          </p>
        </div>
      </section>
                ...
                {summary.subjects.length > 0 && (
                  <div className="mt-5 flex flex-wrap gap-1.5">
                    {summary.subjects.map((s) => (
                      <span
                        key={s}
                        className="rounded-full border border-primary/40 bg-primary/10 px-3 py-0.5 font-mono text-[10px] uppercase tracking-widest text-primary"
                      >
                        {s}
                      </span>
                    ))}
            ...
            <div>
              ...
              <ul className="mt-4 grid gap-2 md:grid-cols-2">
                {result.placement_prep?.map((p, i) => (
                  <li key={i} className="panel rounded-xl px-4 py-3 text-sm">
                    {p}
                  </li>
                ))}
              </ul>
